...
**document-generator / Phase 2 完了 ✅ → Phase 3 着手可能**

Phase 2（バッチ書類生成・案件フォルダ自動作成・パターン→書類自動チェック投入・同名 PDF バージョニング）が動作確認まで完了（2026-05-28）。

次は Phase 3: 明細繰返し対応 + 残契約書（A①/A²/B¹/B²）+ 法務局 5 書類の実装。
最初の着手は **ユーザー作業**（案件一覧に法務局用 5 列追加、物件シートに `不動産番号` 列追加、13 件の【テンプレ】Docs にプレースホルダ書き込み）。
...
| [document-generator](../projects/document-generator/) | 案件確定時の書類自動生成（モノレポの主要案件） | ✅ Phase 2 完了 → Phase 3 着手可能 |
...
### Phase 3: 明細繰返し対応 + 残契約書 + 法務局書類 📅 未着手
...
- [ ] 案件一覧に **法務局用 5 列**（売主生年月日 / 売主旧住所 / 売主新住所 / 住所変更日 / 法務局支局）を追加
- [ ] 物件シートに **`不動産番号` 列** を追加（登記簿 12〜13 桁、登記申請書で必要）
- [ ] 13 件の【テンプレ】Docs に `{{ }}` プレースホルダを書き込み（[`projects/document-generator/docs/documents/`](../projects/document-generator/docs/documents/) 配下の各プレースホルダ案を参照）

#### Claude 実装

- [ ] プレースホルダ規約に `{{#each items}}...{{/each}}` 構文を実装
- [ ] 物件シートのデータを案件 ID で集約 → 明細表として差し込み
- [ ] A①/A②/B①/B② 4 契約書のプレースホルダ置換ロジック
- [ ] 法務局関連 5 書類のプレースホルダ置換ロジック
